...
public class Checkout_JAKR_Test1 {
	Cart cart; 
	...
	public void testChargeDays() {
		assertEquals(2, cart.calculateNumberOfChargeDays(), "Charge days do not include weekend or holiday days for Jackhammer tool type.");
	}

	@Test
	public void testPrediscountCharge() {
		assertEquals(5.98, cart.getTool().getDailyRate() * cart.calculateNumberOfChargeDays(), "Pre-discount charge calculation - daily rental charge * chargable days.");
	}
	...
	public void testDiscountAmount() {
		...
		assertEquals(decimalFormatter.format(0), decimalFormatter.format(prediscountCharge * ((double)cart.getDiscountPercent()/100)), "Discount Amount - Pre-discount charge * discount percentage.");
	}
	...
	public void testFinalCharge() {
		...
		assertEquals(decimalFormatter.format(5.98), decimalFormatter.format(prediscountCharge - discountAmount), "Final Charge - Pre-discount charge - discount amount");
	}
}
