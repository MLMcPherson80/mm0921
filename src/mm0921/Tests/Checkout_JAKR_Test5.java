...
public class Checkout_JAKR_Test5 {
	Cart cart;
	...
	public void testChargeDays() {
		assertEquals(5, cart.calculateNumberOfChargeDays(), "Charge days do not include weekend or holiday days for Jackhammer tool type.");
	}

	@Test
	public void testPrediscountCharge() {
		NumberFormat decimalFormatter = new DecimalFormat("#0.00");
		assertEquals(decimalFormatter.format(14.95), decimalFormatter.format(cart.getTool().getDailyRate() * cart.calculateNumberOfChargeDays()), "Pre-discount charge calculation - daily rental charge * chargable days.");
	}
	...
	public void testFinalCharge() {
		...
		assertEquals(decimalFormatter.format(14.95), decimalFormatter.format(prediscountCharge - discountAmount), "Final Charge - Pre-discount charge - discount amount");
	}
}
