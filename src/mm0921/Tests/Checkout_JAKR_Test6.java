...
public class Checkout_JAKR_Test6 {
	Cart cart;
	...
	public void testChargeDays() {
		assertEquals(1, cart.calculateNumberOfChargeDays(), "Charge days do not include weekend or holiday days for Jackhammer tool type.");
	}

	@Test
	public void testPrediscountCharge() {
		assertEquals(2.99, cart.getTool().getDailyRate() * cart.calculateNumberOfChargeDays(), "Pre-discount charge calculation - daily rental charge * chargable days.");
	}
	...
	public void testDiscountAmount() {
		...
		assertEquals(decimalFormatter.format(1.50), decimalFormatter.format(prediscountCharge * ((double)cart.getDiscountPercent()/100)), "Discount Amount - Pre-discount charge * discount percentage.");
	}
	...
	public void testFinalCharge() {
		NumberFormat decimalFormatter = new DecimalFormat("#0.00");
		double prediscountCharge = cart.getTool().getDailyRate() * cart.calculateNumberOfChargeDays();
		double discountAmount = Double.parseDouble(decimalFormatter.format(((double)cart.getDiscountPercent()/100) * prediscountCharge));
		assertEquals(decimalFormatter.format(1.49), decimalFormatter.format(prediscountCharge - discountAmount), "Final Charge - Pre-discount charge - discount amount");
	}
}
